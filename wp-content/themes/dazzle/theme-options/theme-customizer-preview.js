...
( function( $ ) {
	...
	var $style = $( '#dazzle-customizer-inline-css' ),
		api    = wp.customize;

	// Prepare inline styles in the preview window
	if ( $style.length === 0 ) {
		var style_container = '<' + 'style type="text/css" id="dazzle-customizer-inline-css" />',
			$theme_css      = $( '#dazzle-custom-inline-css' );
		if ($theme_css.length === 0) {
			$theme_css = $( '#dazzle-custom-css' );
		}
		if ($theme_css.length === 0) {
		...
		$style = $( '#dazzle-customizer-inline-css' );
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-other-controls', function( obj ) {
			...
			if (id.indexOf( 'body_style' ) === 0) {
				...
				$( '.sc_layouts_logo .logo_text' ).html( dazzle_prepare_macros( val ) );

			} else if (id == 'blogdescription') {
				$( '.sc_layouts_logo .logo_slogan' ).html( dazzle_prepare_macros( val ) );

			} else if (id == 'copyright') {
				$( '.copyright_text' ).html( dazzle_prepare_macros( val ) );
				...
				if (id == 'front_page_bg_image') {
					...
					if (color_type == 'none') {
						...
						color = typeof dazzle_color_schemes[scheme] != 'undefined' ? dazzle_color_schemes[scheme]['colors']['bg_color'] : '';
					}
					$( '.front_page_section_' + section + '_inner' ).css( 'background-color', color == '' ? 'transparent' : ( mask == 1 ? color : dazzle_hex2rgba( color, mask ) ) );
					...
					.height( 'auto' )
					.toggleClass( 'dazzle-full-height sc_layouts_flex sc_layouts_columns_middle', val );
					$( window ).trigger( 'resize' );
				}
			}
		} );
		...
		function add_color_scheme_helpers() {
			$( '[class*="scheme_"]:not(html)' ).each( function() {
				var $self = $( this ),
					...
					scheme = dazzle_get_class_by_prefix( classes, 'scheme_' ).replace( 'scheme_', '' );
				$self.append( '<div class="dazzle_customizer_scheme_helper">'
									+ '<span class="dazzle_customizer_scheme_helper_name">'
										+ tag
										+ ( id
											? '#' + id
											: ( classes
		...
		function remove_color_scheme_helpers() {
			$( '.dazzle_customizer_scheme_helper' ).remove();
		}
