...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			dazzle_create_observer( 'dazzle-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				dazzle_gutenberg_editor_init();
			} );
		} else {
		 	dazzle_gutenberg_editor_init();
		}
	...
	function dazzle_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'dazzle_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'dazzle_inited' ) ) {
	...
	// Init on page load
	function dazzle_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = dazzle_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + DAZZLE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + DAZZLE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in DAZZLE_STORAGE['override_classes'] ) {
				$obj.addClass( DAZZLE_STORAGE['override_classes'][i].replace( '%s', DAZZLE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( dazzle_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && DAZZLE_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'dazzle_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			dazzle_remove_observer( 'dazzle-check-editor-styles-wrapper' );
			dazzle_create_observer( 'dazzle-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'dazzle_inited' );
	}
} );
