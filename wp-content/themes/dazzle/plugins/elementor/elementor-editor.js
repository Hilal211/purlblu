...
jQuery( document ).ready(
	function() {
		...
		dazzle_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="dazzle_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.dazzle_post_editor' );
						if ( link.length > 0 ) {
							dazzle_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.dazzle_post_editor', function(e) {
						dazzle_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('dazzle_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function dazzle_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
