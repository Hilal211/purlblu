...
if ( apply_filters( 'dazzle_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
    $dazzle_post_format = str_replace( 'post-format-', '', get_post_format() );
    $post_meta = in_array( $dazzle_post_format, array( 'video' ) ) ? get_post_meta( get_the_ID(), 'trx_addons_options', true ) : false;
	...
	ob_end_clean();
	$dazzle_with_featured_image = dazzle_is_with_featured_image( $dazzle_post_header /*, array( 'with_gallery' )*/);
	// Post title and meta
	ob_start();
	...
	if ( strpos( $dazzle_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $dazzle_with_featured_image ) {
                echo ' with_featured_image' . ( false && dazzle_get_theme_option( 'single_parallax' ) == 0 ? ' dazzle-full-height' : '' );
            }
		?>">
			<?php
