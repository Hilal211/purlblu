...
if ( function_exists( 'dazzle_elementor_is_preview' ) && dazzle_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			dazzle_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	dazzle_new_main_query(
		array(
			'post_type'      => dazzle_get_theme_option( 'post_type' ),
			'category'       => dazzle_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'dazzle_filter_get_template_part', dazzle_blog_archive_get_template() ) );
}
